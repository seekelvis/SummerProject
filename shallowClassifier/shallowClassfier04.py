...
def model_build(classifier, trian_features, train_labels, test_features):
    ...
    if classifier=="IW":
        ...
        # pipe = make_pipeline(StandardScaler(), ImportanceWeightedClassifier(kernel_type="rbf"))
        # param_grid = [{}]
        # model = GridSearchCV(pipe, param_grid, cv=3)
        # model.fit(trian_features, train_labels, test_features)
        # print(model.best_params_)

        model = ImportanceWeightedClassifier().fit(trian_features, train_labels, test_features)
...
def tsne_show(x_domain,y_domain):
    tsne = TSNE(n_components=2)
    x_embedded = tsne.fit_transform(x_domain)
    y_embedded = tsne.fit_transform(y_domain)

    plt.figure()
    plt.title("source_domain_T-SNE")
    plt.scatter(x_embedded[:, 0], x_embedded[:, 1])
    fig1 = plt.gcf()
    fig1.show()

    plt.figure()
    plt.title("target_domain_T-SNE")
...
if __name__ == '__main__':
    ...
    # tsne_show(train_features_source,test_features_target)
